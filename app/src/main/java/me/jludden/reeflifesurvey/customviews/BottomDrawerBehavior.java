...
/**
 * Created by Jason on 1/25/2018.
 *
 * A slightly modified copy of {@link BottomSheetBehavior}
 * Allows the bottom sheet to be open or expanded with a gesture up on any view in the Coordinator layout
 * Some slight modifications to make this behavior work better on an {@link ExtendedViewPager} holding a {@link TouchImageView}
 */

public class BottomDrawerBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    public abstract static class BottomSheetCallback {
         ...
         */
        public abstract void onStateChanged(@NonNull View bottomSheet, @BottomDrawerBehavior.State int newState);
    ...
    private BottomDrawerBehavior.BottomSheetCallback mCallback;
     ...
     */
    public BottomDrawerBehavior(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    //jludden - overriding default google behavior here, still accepting touch events if the view is hidden
    @Override
    ...
    /*
        jludden - overriding default google behavior here:
        we still intercepting touch events if the child is hidden
          and not ignoring them, even if the point is not in the child's bounds
        we also don't want to intercept touch if it is part of a multifinger gesture
     */
    @Override
        ...
        switch (action) {
            ...
            case MotionEvent.ACTION_DOWN:
               ...
               /* mIgnoreEvents = mActivePointerId == MotionEvent.INVALID_POINTER_ID;*//* &&
                ...
                break;
            case MotionEvent.ACTION_POINTER_DOWN: //multifinger
                mIgnoreEvents = true;
                return false;
        }
     ...
     */
    public void setBottomSheetCallback(BottomDrawerBehavior.BottomSheetCallback callback) {
        mCallback = callback;
    }
     ...
     *              {@link #STATE_HIDDEN}.
     */
    public final void setState(final @BottomDrawerBehavior.State int state) {
        if (state == mState) {
            return;
        }
        if (mViewRef == null) {
    ...
    void setStateInternal(@BottomDrawerBehavior.State int state) {
        if (mState == state) {
            return;
        }
        mState = state;
    ...
    private final ViewDragHelper.Callback mDragCallback = new ViewDragHelper.Callback() {
        ...
        //jludden - overriding default google behavior here (if drag is released with the point still moving up, expand)
        //  when the drag is released, we want to to pop into collapsed mode or expanded mode based on the top of the view
        @Override
        public void onViewReleased(View releasedChild, float xvel, float yvel) {
            int top;
            @BottomDrawerBehavior.State int targetState;
            /*if (yvel < 0) { // Moving up
                top = mMinOffset;
                targetState = STATE_EXPANDED;
            } else */
    ...
    private class SettleRunnable implements Runnable {
        ...
        SettleRunnable(View view, @BottomDrawerBehavior.State int targetState) {
            mView = view;
    ...
    protected static class SavedState extends AbsSavedState {
        ...
        public SavedState(Parcelable superState, @BottomDrawerBehavior.State int state) {
            super(superState);
            this.state = state;
        }
    ...
    /**
     * A utility function to get the {@link BottomDrawerBehavior} associated with the {@code view}.
     *
     * @param view The {@link View} with {@link BottomDrawerBehavior}.
     * @return The {@link BottomDrawerBehavior} associated with the {@code view}.
     */
    @SuppressWarnings("unchecked")
    public static <V extends View> BottomDrawerBehavior<V> from(V view) {
        ...
        CoordinatorLayout.Behavior behavior = ((CoordinatorLayout.LayoutParams) params)
                .getBehavior();
        if (!(behavior instanceof BottomDrawerBehavior)) {
            throw new IllegalArgumentException(
                    "The view is not associated with BottomSheetBehavior");
        }
        return (BottomDrawerBehavior<V>) behavior;
    }
